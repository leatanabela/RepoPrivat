    ...
    <div className="flex-1 overflow-y-auto">
      <div className="p-6 lg:p-10 max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-primary">Tichetele Mele</h1>
            <p className="text-slate-500 mt-1">Urmareste starea cererilor tale de asistenta</p>
          </div>
          <Link
            href="/tickets/new"
        ...
        <div className="bg-white dark:bg-background-dark rounded-xl border border-primary/10 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center text-slate-500">
              <p className="text-lg font-medium">Nu exista tichete</p>
              <p className="text-sm mt-1">Creeaza primul tau tichet pentru a primi asistenta.</p>
            </div>
          ) : (
                        ...
                        <Link href={`/tickets/${ticket.id}`} className="block">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-400">
                              {ticket.departments?.name || 'General'} {ticket.ticket_categories?.name ? `• ${ticket.ticket_categories.name}` : ''}
                            </span>
                          </div>
                        </Link>
                      </td>
